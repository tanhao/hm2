...
public class AccessLogFilter implements GlobalFilter, Ordered {
    @Autowired
    GatewayLogServer gatewayLogServer;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        //过滤拦截列表
        if (request.getMethodValue().equals(HttpMethod.GET.toString())) {
            return chain.filter(exchange);
        }
        Route route = getGatewayRoute(exchange);
        //过滤order服务器，不需要进行日志记录
        if (route.getId().equals("hm2-service-order") || route.getId().equals("hm2-service-member")) {
            return chain.filter(exchange);
        }
    ...
    private void writeAccessLog(GatewayLog gatewayLog) {
        gatewayLogServer.insertSelective(gatewayLog);
    }
        ...
        return new ServerHttpResponseDecorator(response) {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                if (body instanceof Flux) {
                    ...
                    gatewayLog.setCreateDay(responseTime);
                    // 计算执行时间
