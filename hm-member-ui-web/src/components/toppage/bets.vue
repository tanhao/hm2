<template>
  <div style="margin:0px 5px 0px 0px;padding-bottom: 5px">
    <select id="kjlist" name="status" v-model="params.status" @change="getBetList">
      <option value="WAITING">未结</option>
      <option value="CANCEL">撤单</option>
    </select>
    <input type="date" ref="selectDate" @change="getBetList" :max="currentDate.toString()"></input>
      <!--未结已结样式通用-->
      <table class="list table">
        ...
        <tr>
          ...
          <th>下注金额</th>
          <th>退水(%)</th>
          <th v-show="params.status=='WAITING'">可赢金额</th>
        </tr>
        </thead>
        <tbody>
          ...
          <tr class="">
            ...
            <td class="contents">
              <template class="blue_color" v-if="item.lotteryId==101 || item.lotteryId==102 || item.lotteryId==103 || item.lotteryId==104 ||
                  ...
                  <span >{{$t(JSON.parse(item.keyName).playKey)}}
            ...
            <td>{{item.commPct}}%</td>
            <td v-show="params.status=='WAITING'" class="result color">{{item.winAmt | moneyFmt}}</td>
          </tr>
        </template>
        </tbody>
        <tfoot>
        <tr>
          ...
          <td></td>
          <td v-show="params.status=='WAITING'" class="result color">{{pageOrderWin | moneyFmt }}</td>
        </tr>
        </tfoot>
      </table>
      <!--结束
...
<!--公用分页样式,完成后放回最下面-->
    <div id="nav">
      <ul class="pagination margin">
        ...
        <li @click="selectPage(params.page-1)"><a href="javascript:void(0)">❮</a></li>
        <template v-for="(item,index) in pageList">
          <li @click="selectPage(item)"><a href="javascript:void(0)" :class="item==params.page?'active':''">{{item}}</a></li>
        </template>
