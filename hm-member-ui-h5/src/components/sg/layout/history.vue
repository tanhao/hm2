<template>
  <div class="home">
    <div class="sc-bZQynM OQRyf gm_main">
      <my-header title="两周报表" back="true" left="true"></my-header>
      <div class="sc-htoDjs UDzZc">
        <div class="block">
          <div class="segmented segmented-round">
            <button @click="weekSelectFun(true)" class="button button-round button-outline "
                    :class="weekActive?'button-active':''">两周报表
            </button>
            <button @click="weekSelectFun(false)" class="button button-round button-outline"
                    :class="!weekActive?'button-active':''">作废注单
            </button>
          </div>
        </div>
        ...
        <div class="block">
          <div class="segmented segmented-round">
            <button @click="nowPrevActive(true)" class="button button-round button-outline "
                    :class="nowOrPrevActive?'button-active':''">本周
            </button>
            <button @click="nowPrevActive(false)" class="button button-round button-outline"
                    :class="!nowOrPrevActive?'button-active':''">上周
            </button>
          </div>
        </div>
        <div id="top-line"></div>
        <div :class="weekActive?'table':'table line-through'">
          <div class="table-header">
            <div class="col col3">日期</div>
            <div class="col col2">注数</div>
            <div class="col col2">下注金额</div>
            <div class="col col2">有效金额</div>
            <div class="col col2">退水</div>
            <div class="col col2">盈亏</div>
          </div>

          <div class="table-content">
            <template v-for="(list,i) in nowList">
              <a class="table-row" v-show="nowOrPrevActive" @click="selectSameDay(list.date)">
                <div class="col col3 green_color">{{list.date}}
                  <br/>
                  {{$t(list.week)}}
                </div>
                <div class="col col2">{{list.num}}</div>
                <div class="col col2">{{list.betAmt }}</div>
                <div class="col col2">{{list.actAmt }}</div>
            ...
            <template v-for="(list,i) in prevList">
          ...
          <div class="table-footer">
            ...
            <div class="col col2">{{parseInt(totalActAmt)}}</div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['gameMenu', 'showMenu', 'gameId', 'game', 'historyHomeReturn']),
    ...
    ,
    methods: {
      ...mapActions(['setHistoryReturn', 'setHistoryHomeReturn']),
      weekSelectFun(flag) {
      ...
      nowPrevActive(flag) {
        ...
        if (flag) {
          for (let i = 0; i < self.nowList.length; i++) {
            self.totalNum = Utils.NumberAdd(self.nowList[i].num, self.totalNum);
            self.totalBetAmt = Utils.NumberAdd(self.nowList[i].betAmt, self.totalBetAmt);
            self.totalActAmt = Utils.NumberAdd(self.nowList[i].actAmt, self.totalActAmt);
            self.totalComm = Utils.NumberAdd(self.nowList[i].comm, self.totalComm);
            self.totalWinAmt = Utils.NumberAdd(self.nowList[i].winAmt, self.totalWinAmt);
          }
        } else {

          for (let i = 0; i < self.prevList.length; i++) {
            self.totalNum = Utils.NumberAdd(self.prevList[i].num, self.totalNum);
            self.totalBetAmt = Utils.NumberAdd(self.prevList[i].betAmt, self.totalBetAmt);
            self.totalActAmt = Utils.NumberAdd(self.prevList[i].actAmt, self.totalActAmt);
            self.totalComm = Utils.NumberAdd(self.prevList[i].comm, self.totalComm);
            self.totalWinAmt = Utils.NumberAdd(self.prevList[i].winAmt, self.totalWinAmt);
          }
        }
        this.nowOrPrevActive = flag;
      },
      selectSameDay(date) {
        let params = {selectDate: date};
        if (!this.weekActive) {
          this.$set(params, "status", 'VOID');
        }
        this.$router.push({path: '/sg/samedayhistory', query: params});
      },
      weekReportGet(params) {
        let self = this;
        Indicator.open({text: '加载中...'});
        Lottery.getReport(params).then(val => {
          if (val.code == 10000) {
    ...
    mounted() {
      let self = this;
      this.weekReportGet();
      //self.setHistoryReturn({'name':self.historyHomeReturn,'query':{},'mode':0});
