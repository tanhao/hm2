<template>
  <div class="home">
    <div class="sc-bZQynM OQRyf gm_main">
      <my-header title="已结明细" left="false" resbnt="true" @refreshPageFun="getBetList"></my-header>
...
<!--                  <div class="blue_color" v-if="item.lotteryId==401 || item.lotteryId==402 || item.lotteryId==403
                ...
                <div class="col col1 blue_color">
                  <template v-if="parseFloat(item.winAmt) >= 0">
                    <span class="blue_color">{{item.winAmt | moneyFmt}}</span>
                  ...
                  <div class="red_color">
                    <span class="blue_color">{{item.water}}</span>

                  </div>
                  <div v-if="item.status=='REDIVIDEND'">
                    <span style="color:#302F2F">重派</span>
          ...
          <div class="table-footer pagination">
            <div class="pagination sc-gzVnrw oPkft">
              <div style="display: initial; left: 66px;top: -35px; position: absolute;">共<span style="margin: 0 8px;color: #0698b1;font-weight: bold">{{totalPage}}</span>页 </div>
              <div class="page-navi">
                <span class="prev disabled" @click="pageDownOrUp(params.page-1)"><a ></a></span>
                <span class="current">

                  <form class="page-jump">
                    <input type="num" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="inputPages" v-model="showPage" >
                  </form><span>页</span></span>
                <span class="next disabled" @click="pageDownOrUp(params.page+1)"><a ></a></span>
              </div>
            </div>
            <div>
              <div class="footer-inline"><div class="title" >注数(总计)</div><span class="content" >{{this.orderTotal}}</span></div>
              <div class="footer-inline"><div class="title">本页下注金额</div><span class="content">{{this.pageOrderMoney | moneyFmt}}</span></div>
              <div class="footer-inline"><div class="title">本页结果</div><!--<span class="content">
                {{this.pageOrderWin | moneyFmt}}</span>-->
                <template v-if="parseFloat(this.pageOrderWin) >= 0">
                  <span class="content blue_color">{{this.pageOrderWin | moneyFmt}}</span>

                </template>
                <template v-if="parseFloat(this.pageOrderWin) < 0">
                  <span class="content red_color">{{this.pageOrderWin | moneyFmt}}</span>
