<template>
  <div class="home">
    <div class="sc-bZQynM OQRyf gm_main">
      <my-header title="开奖结果" left="false"></my-header>
          ...
          <template v-for="(item,i) in results">
            <template v-if="item.result!=null && item.result!=''">
              <div class="sc-cSHVUG cfhhpx" v-if="item.lotteryId==101 || item.lotteryId==102 ||
                    ...
                    item.lotteryId==105 || item.lotteryId==106 || item.lotteryId  == 107||
                    item.lotteryId==108 || item.lotteryId == 109 || item.lotteryId==110 || item.lotteryId==111
                  || item.lotteryId==112">
                <div class="sc-kGXeez ljfNBe">
              ...
              <div class="sc-cSHVUG cfhhpx" v-else-if="item.lotteryId==201 || item.lotteryId==203 || item.lotteryId==202
                  || item.lotteryId==204 || item.lotteryId==205 || item.lotteryId==206 || item.lotteryId==207
              ...
              <div class="sc-cSHVUG cfhhpx" v-else-if="item.lotteryId==401 || item.lotteryId==402 || item.lotteryId==403 ">
                ...
                <div class="sc-kGXeez kbmxrg jsdd" id="list">
                  ...
                  <span :class="'n_'+(Number.parseInt(item.result[0])+Number.parseInt(item.result[1])+Number.parseInt(item.result[2]))">{{(Number.parseInt(item.result[0])+Number.parseInt(item.result[1])+Number.parseInt(item.result[2]))}}</span>
                </div>
              </div>
              <div class="sc-cSHVUG cfhhpx" v-else-if="item.lotteryId==301 || item.lotteryId==302 || item.lotteryId ==303 || item.lotteryId ==304">
                ...
                <div :class="params.lotteryId==303?'sc-kGXeez kbmxrg xync':'sc-kGXeez kbmxrg klsf'" id="list">
                  <template v-for="(obj,index) in item.result">
                    <div :class="'n_'+obj">
                      <span>{{obj}}
                      </span>
              ...
              <div class="sc-cSHVUG cfhhpx" v-if="item.lotteryId==701">
                <div class="sc-kGXeez ljfNBe">
                  <span class="title">{{item.gameNo}} </span>
                  <span class="title">{{item.actionTime*1000 | formatDate}} </span>
                </div>
                <div class="sc-kGXeez kbmxrg gxkl10" id="list">
                  <template v-for="(obj,index) in item.result">
                    <div :class="'n_'+obj">
                      <span >{{obj}}
                      </span>
              ...
              <div class="sc-cSHVUG cfhhpx" style="height:54px" v-if="item.lotteryId==501 || item.lotteryId==502">
                <div class="sc-kGXeez ljfNBe">
                  <span class="title">{{item.gameNo}} </span>
                  <span class="title">{{item.actionTime*1000 | formatDate}} </span>
                </div>
                <div id="listkl8" class="sc-kGXeez kbmxrg kl8">
                  <template v-for="(obj,index) in item.result">
                    <div :class="'n_'+obj">
                      <span >{{obj}}
                      </span>
              ...
              <div class="sc-cSHVUG cfhhpx" v-if="item.lotteryId==601">
                <div class="sc-kGXeez ljfNBe">
                  <span class="title">{{item.gameNo}} </span>
                  <span class="title">{{item.actionTime*1000 | formatDate}} </span>
                </div>
                <div class="sc-kGXeez kbmxrg gd11x5" id="list">
                  <template v-for="(obj,index) in item.result">
                    <div :class="'n_'+obj">
                      <span >{{obj}}
                      </span>
    ...
    <my-footer @reloadFun="reloadFun"></my-footer>
  </div>
</template>
<script>
  ...
  export default {
    ...
    filters: {
      formatDate(time) {
        ...
        return Utils.formatDate(date, 'hh:mm:ss'); // 年月日 格式自己定义   'yyyy : MM : dd'  例 2018年12月5日的格式
      },
      resultFilter(val){
        let obj = '';
        if(val.at(0)==='0'){
    ...
    methods:{
      ...
      async getLotteryResult(){
        ...
        let self = this;
        let [err,data] = await to(Lottery.lotteryResult(self.params));
        if(data.success){
