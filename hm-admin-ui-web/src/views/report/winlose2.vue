        ...
        methods: {
                ...
                this.$api.order.getWinLose(this.params).then(res => {
                    if (res.success) {
                        ...
                        if (this.infoList.length > 0) {
                            this.convertMinus(this.infoList);
            ...
            loadWinLoseByKind() {
                ...
                this.$api.order.getWinLoseByKind(this.params).then(res => {
                    if (res.success) {
                        let kinds = Object.assign([], res.data.kinds);
                        this.convertMinus(kinds);
                        kinds.forEach((item) => {
            ...
            convertMinus(selection){
                for (let col of selection) {
                    col.toLowerWin=col.toLowerWin*-1;
                    col.toLowerComm=col.toLowerComm*-1;
                    col.actualWinAmt=col.actualWinAmt*-1;
                    col.actualCommAmt=col.actualCommAmt*-1;
                    col.actualResult=col.actualResult*-1;
                    col.gainOddsAmt=col.gainOddsAmt*-1;
                    col.gainCommOdds=col.gainCommOdds*-1;
                    col.gainCommAmt=col.gainCommAmt*-1;
                }
            },
            handTotal(selection) {
