<template>
    <a-modal :title="'修改限注日志：'+editUsername" :width="1030" :maskClosable="maskClosable" dialogClass="tk" :visible="regressLimitLogShow" :body-style="{ paddingBottom: '0px'}" @cancel="onClose">
...
export default {
    ...
    methods: {
        ...
        onClose() {
            this.$emit("update:regressLimitLogShow", false);
        },
        async requestLog() {
            ...
            let [err, data] = await to(this.$api.ctrl.getLogLimit(params));
            if (err || !data.success) {
