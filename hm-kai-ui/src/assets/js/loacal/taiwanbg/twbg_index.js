		...
		$("#videobox").animate({
			...
			$(".content").animate({
				...
				if(seconds == 0) { //当首页为开奖中...时点击视频，只为期填充上一期数据，且一直处于开奖中动画状态\
					...
					$("iframe")[0].contentWindow.syxwV.pcFillData(dataStr.drawIssue, dataStr.drawTime, preNunArr, 1); //更新当前本期开奖号码
					$("iframe")[0].contentWindow.syxwV.intoKai(); //进入模拟开奖界面动画
					$("iframe")[0].contentWindow.syxwV.startFun();
				} else {
